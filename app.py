...
Ứng dụng Web phát hiện gian lận dựa trên Logistic Regression, XGBoost & Beneish M-Score
...
# ==============================================================================
# 4. HUẤN LUYỆN VÀ CACHE CÁC MÔ HÌNH HỌC MÁY
# ==============================================================================
...
@st.cache_resource(show_spinner="Đang tải dữ liệu và huấn luyện mô hình XGBoost...")
def train_xgboost_model(data_path=DEFAULT_CSV):
    """Đọc dữ liệu CSV và huấn luyện pipeline XGBoost trên cùng tập chia với Logistic."""
    ...
    if len(features) != len(EXPECTED_FEATURES):
        ...
        raise ValueError(f"Dữ liệu huấn luyện thiếu các biến: {missing}")
    ...
    y_prob_xgb = xgb_pipeline.predict_proba(X_test_xgb)[:, 1]

    importance_df = pd.DataFrame(
        {
            "Biến số": features,
            "Tên tiếng Việt": [FEATURE_INFO[f]["vn_name"] for f in features],
            "Mức độ quan trọng": xgb_pipeline.named_steps["model"].feature_importances_,
        }
    ).sort_values("Mức độ quan trọng", ascending=False)

    return xgb_pipeline, y_prob_xgb, importance_df
...
with st.sidebar:
    ...
    selected_ml_model = st.selectbox(
        ...
        help="XGBoost phù hợp với quan hệ phi tuyến; Logistic Regression dễ diễn giải hệ số.",
    )
    threshold = st.slider(
    ...
    st.markdown(
        ...
        2. **Mô hình XGBoost (Machine Learning):**
           - Học các quan hệ phi tuyến và tương tác giữa các chỉ số.
           - Tự xử lý mất cân bằng lớp trong tập huấn luyện.
        3. **Mô hình Beneish M-Score Chuẩn (1999):**
           - Tiêu chuẩn toàn cầu trong kiểm toán.
           - Ngưỡng phân định: $M > -1.78$.
        """
    )
    st.markdown("---")
    st.caption("Phiên bản 1.1 | Logistic + XGBoost + Beneish M-Score")

# ==============================================================================
# 6. KHỞI TẠO MÔ HÌNH
# ==============================================================================
pipeline, X_train, X_test, y_train, y_test, y_prob, coef_df, intercept_raw = train_logistic_model(DEFAULT_CSV)
xgb_pipeline, y_prob_xgb, xgb_importance_df = train_xgboost_model(DEFAULT_CSV)
...
if XGBClassifier is None:
    st.error(
        "⚠️ Chưa cài thư viện XGBoost. Hãy thêm `xgboost` vào requirements.txt "
        "hoặc chạy `pip install xgboost`, sau đó khởi động lại ứng dụng."
    )
    st.stop()

if xgb_pipeline is None:
    st.error("⚠️ Không thể khởi tạo mô hình XGBoost từ dữ liệu huấn luyện.")
    st.stop()

active_y_prob = y_prob_xgb if selected_ml_model == "XGBoost" else y_prob
...
with tab1:
    ...
    # 1. Dự báo từ hai mô hình học máy
    prob_logistic = pipeline.predict_proba(input_data)[0, 1]
    prob_xgb = xgb_pipeline.predict_proba(input_data)[0, 1]
    prob_fraud = prob_xgb if selected_ml_model == "XGBoost" else prob_logistic
    is_logistic_fraud = prob_logistic >= threshold
    is_xgb_fraud = prob_xgb >= threshold
    is_active_fraud = prob_fraud >= threshold
        ...
        if is_logistic_fraud:
            st.markdown(
                ...
                <div class="metric-card-danger">
                    ...
                    <h2 style='color:#dc2626; margin:8px 0;'>{prob_logistic*100:.1f}%</h2>
            ...
            st.markdown(
                ...
                <div class="metric-card-success">
                    ...
                    <h2 style='color:#16a34a; margin:8px 0;'>{prob_logistic*100:.1f}%</h2>
    ...
    with res_col2:
        st.markdown("**Mô Hình XGBoost**")
        if is_xgb_fraud:
            st.markdown(
                f"""
                <div class="metric-card-danger">
                    <span class="badge-danger">NGUY CƠ GIAN LẬN CAO</span>
                    <h2 style='color:#dc2626; margin:8px 0;'>{prob_xgb*100:.1f}%</h2>
                    <p style='color:#6b7280; font-size:0.9rem; margin:0;'>Xác suất XGBoost vượt ngưỡng quyết định ({threshold:.2f}).</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                ...
                <div class="metric-card-success">
                    <span class="badge-safe">AN TOÀN / RỦI RO THẤP</span>
                    <h2 style='color:#16a34a; margin:8px 0;'>{prob_xgb*100:.1f}%</h2>
                    <p style='color:#6b7280; font-size:0.9rem; margin:0;'>Xác suất XGBoost dưới ngưỡng cảnh báo ({threshold:.2f}).</p>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    st.markdown(f"#### Thước đo mô hình đang chọn: {selected_ml_model}")
    ...
    with gauge_col2:
        ...
        fig_gauge = go.Figure(
            go.Indicator(
                ...
                gauge={
                    "axis": {"range": [0, 100]},
                    "bar": {"color": "#ef4444" if is_active_fraud else "#22c55e"},
                    "steps": [
    ...
    with radar_col1:
        ...
        fig_radar.add_trace(
            go.Scatterpolar(
                ...
                line=dict(color="#ef4444" if is_active_fraud else "#2563eb", width=2),
            )
        )
        fig_radar.add_trace(
            go.Scatterpolar(
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_cols:
                ...
                # Dự báo xác suất bằng cả Logistic Regression và XGBoost
                X_batch = batch_df[EXPECTED_FEATURES]
                batch_probs_logistic = pipeline.predict_proba(X_batch)[:, 1]
                batch_probs_xgb = xgb_pipeline.predict_proba(X_batch)[:, 1]
                batch_probs = (
                    batch_probs_xgb
                    if selected_ml_model == "XGBoost"
                    else batch_probs_logistic
                )
                batch_preds_logistic = (batch_probs_logistic >= threshold).astype(int)
                batch_preds_xgb = (batch_probs_xgb >= threshold).astype(int)
                ...
                results_df["XAC_SUAT_LOGISTIC"] = np.round(batch_probs_logistic, 4)
                results_df["DU_BAO_LOGISTIC"] = np.where(batch_preds_logistic == 1, "Rủi ro cao", "An toàn")
                results_df["XAC_SUAT_XGBOOST"] = np.round(batch_probs_xgb, 4)
                results_df["DU_BAO_XGBOOST"] = np.where(batch_preds_xgb == 1, "Rủi ro cao", "An toàn")
                results_df["MO_HINH_DANG_CHON"] = selected_ml_model
                results_df["XAC_SUAT_MO_HINH_CHON"] = np.round(batch_probs, 4)
...
# ==============================================================================
with tab3:
    st.subheader(f"Đánh Giá Hiệu Năng {selected_ml_model} (Test Set 20%)")

    # Tính toán các chỉ số trên test set
    y_test_pred = (active_y_prob >= threshold).astype(int)
    ...
    auc = roc_auc_score(y_test, active_y_prob)

    # Bảng so sánh hai mô hình trên cùng tập kiểm tra và cùng ngưỡng
    comparison_rows = []
    for model_name, model_prob in [
        ...
        comparison_rows.append(
            {
                ...
                "Specificity": model_tn / (model_tn + model_fp) if (model_tn + model_fp) else 0.0,
                "F1-Score": f1_score(y_test, model_pred, zero_division=0),
    ...
    st.caption(
        f"*Đang hiển thị {selected_ml_model}. Kết quả phân loại thay đổi theo ngưỡng "
        f"(Threshold = {threshold:.2f}) thiết lập ở thanh bên; ROC-AUC không phụ thuộc ngưỡng.*"
    )

    st.markdown("#### So Sánh Logistic Regression Và XGBoost")
    st.dataframe(
        comparison_df.style.format(
            ...
            subset=["Accuracy", "Precision", "Recall", "Specificity", "F1-Score", "ROC-AUC"],
            color="#dcfce7",
        ),
        use_container_width=True,
        hide_index=True,
    ...
    with chart_col2:
        ...
        fig_roc = go.Figure()
        fig_roc.add_trace(go.Scatter(x=fpr_log, y=tpr_log, mode="lines", name=f"Logistic (AUC = {auc_log:.3f})", line=dict(color="#2563eb", width=2.5)))
        fig_roc.add_trace(go.Scatter(x=fpr_xgb, y=tpr_xgb, mode="lines", name=f"XGBoost (AUC = {auc_xgb:.3f})", line=dict(color="#ef4444", width=2.5)))
    ...
    st.markdown("---")
    st.subheader("Trọng Số & Mức Độ Quan Trọng Của Các Biến")
    ...
    st.markdown("#### Mức Độ Quan Trọng Của Biến Trong XGBoost")
    st.dataframe(
        xgb_importance_df.style.format({"Mức độ quan trọng": "{:.4f}"}),
        use_container_width=True,
        hide_index=True,
    )
    fig_xgb_importance = px.bar(
        xgb_importance_df.sort_values("Mức độ quan trọng", ascending=True),
        x="Mức độ quan trọng",
        y="Biến số",
        orientation="h",
        color="Mức độ quan trọng",
        color_continuous_scale="Viridis",
        title="Feature Importance Của Mô Hình XGBoost",
        text="Mức độ quan trọng",
    )
    fig_xgb_importance.update_traces(texttemplate="%{text:.3f}", textposition="outside")
    fig_xgb_importance.update_layout(height=400)
...
with tab4:
    ...
    st.markdown(
        ...
        3. **Đối chiếu chéo 3 mô hình**: Kết hợp Logistic Regression, XGBoost và Beneish M-Score để đưa ra nhận định toàn diện hơn; không sử dụng kết quả mô hình thay cho kết luận kiểm toán.
        """
    )
